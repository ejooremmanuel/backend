...
const verrifyGoogleTokens = async (request, response, next) => {
  ...
  const { refresh_token, access_token, access_token_expires, authorization } =
    request.headers || request?.user?.googleTokens || {};
  ...
  if (
    ...
    isExpired(access_token_expires)
  ) {
...
const getUserChannelInfo = async (request, response, next) => {
  const { refresh_token, access_token, access_token_expires } =
    request?.user?.googleTokens || {};
  ...
  const [channelInfo, channelError] = await promiseWrapper(
    getChannelInfoOfToken(access_token)
  );
  const channelId =
    channelInfo?.items?.length > 0 ? channelInfo.items[0]?.id : null;
  const playlistId =
    channelInfo?.items?.length > 0
...
const youtubeChannelRequired = (request, response, next) => {
  if (
    ...
    return response.status(401).json({
      error: "Google oauth is required or you don't have a youtube account",
    });
  }
