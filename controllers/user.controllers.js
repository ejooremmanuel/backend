...
const uploadProfileImage = async (request, response) => {
  ...
  const [uploadedUrl, error] = await promiseWrapper(
    uploadFile(
      `/avatars/${randomStr(20)}.${ext}`,
      resizedFile || files.avatar.path
    )
  );
...
const updateProfile = async (request, response) => {
  ...
  if (errors.length > 0)
    return response.status(400).json({ errors, ok: false });

  let { name } = body;
  if (name && name.length <= 3)
    errors.push("The name field should be more than 3 chars");
